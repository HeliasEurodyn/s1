...
namespace ClassLibrary8
{
    class ExportXmlFileAMEN
    {
      ...
      String FTPPASSWORD
      )
        {
            ...
            try
            {
                ...
                Files00200101C.XmlIOClassC xmlC = new Files00200101C.XmlIOClassC();
                ...
                for (int i = 0; i < tbl.Count; i++)
                {
                    ...
                    Files00200101C.MndtInfrm mndtInfrm = new Files00200101C.MndtInfrm();


                    mndtInfrm.GrpHdr.MessageIdentification = DateTimeStamp + "-" + tbl[i, "TRDR"].ToString() + "-" + tbl[i, "SIGNATUREID"].ToString() + "-1";  // BigCustomerText + DateTime.Now.ToString("yyMMdd") + AINCFile;
                  ...
                  //  mndtInfrm.GrpHdr.dbtrAgt.finInstnId.BIC = tbl[i, "BIC"].ToString();
                    ...
                    mndtInfrm.GrpHdr.TxCd = "AMEN";
                    mndtInfrm.GrpHdr.pmtTpInf.svcLvl.Cd = "SEPA";
                    mndtInfrm.GrpHdr.pmtTpInf.lclInstrm.Cd = "B2B";


                    Files00200101C.XMLPaymentInformationC mndtInf = new Files00200101C.XMLPaymentInformationC();

                    mndtInf.TxId = DateTimeStamp + "-" + tbl[i, "TRDR"].ToString() + "-" + tbl[i, "SIGNATUREID"].ToString() + "-1";  //tbl[i, "FINCODE"].ToString();
                    mndtInf.PmtMtd = "DD";
                    mndtInf.SeqTp = "RCUR";

                 //   String FINALDATE = tbl[i, "FINALDATE"].ToString();
                  //  mndtInf.DtOfSgntr = DateTime.UtcNow.ToString("yyyy-MM-dd");  //FINALDATE;
                    mndtInf.MndtId = DateTimeStamp + "-" + tbl[i, "TRDR"].ToString() + "-" + tbl[i, "SIGNATUREID"].ToString() + "-1"; 
                    mndtInf.OrgnlMndtId = tbl[i, "MANDID"].ToString();
                 ...
                 //   mndtInf.dbtr.Nm = tbl[i, "X_TNAME"].ToString();
                  ...
                  //  mndtInf.dbtrAcct.dbtrAcctId.IBAN = tbl[i, "IBAN"].ToString();
                    ...
                    String SqlUpdate = " UPDATE CCCDIASMAND SET MANDID = '" +
                          DateTimeStamp + "-" + tbl[i, "TRDR"].ToString() + "-" + tbl[i, "SIGNATUREID"].ToString() + "-1"
                ...
                {
                    JobDone = true;
                    XmlSerializer SerializerObj = new XmlSerializer(typeof(Files00200101C.XmlIOClassC));
      ...
      public void insertDateTimeStamp(int AINCFile, String FileName)
      {
          ...
          CCCDIASFOLDERDATELOGTable.Current["MSGID"] = 2;
          CCCDIASFOLDERDATELOGTable.Current["MSGDESCR"] = "Αρχείο Μεταβολών";
